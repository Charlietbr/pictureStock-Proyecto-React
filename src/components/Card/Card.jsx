    ...
    <article className='card'>
        ...
        <section className='imgInfo'>
            ...
            <h4 id='expandir'>▼ Info</h4>
              ...
              <section id='user'>
                <Link to={`/users/${foto.user.username}`}><img className='grow' src={foto.user.profile_image.medium} alt="Ver perfil del creador"/></Link>
                
                <h4>{foto.user.name} ({foto.user.username})</h4>
                <p className='grow'>{foto.user.portfolio_url ? <a href={foto.user.portfolio_url}>Ver su portfolio</a> : ''}</p>
